...
@section('content')
    <div class="container-fluid">
        <div class="card mt-4">
            <div class="card-body">
                <h4 class="card-title">Edit Product Sub Category</h4>
                <form class="form-horizontal p-t-20"
                    action="{{ route('admin.productSubCategory.update', $productSubCategory->id) }}" method="POST">
                    @method('PUT')
                    @csrf
                                ...
                                <div class="col-sm-12">
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="uname1" name="name"
                                            ...
                                            value="{{ $productSubCategory->name ?? null }}">
                                    </div>
                                </div>
                            </div>
                        @else
                                ...
                                <div class="col-sm-12">
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="uname1" name="team_name_bangla"
                                            value="{{ $productSubCategory->name_bangla ?? null }}">
                                    </div>
                                </div>
                            </div>
                                ...
                                <select type="select" class="form-control" id="uname1" name="product_category_id"
                                    ...
                                    @foreach ($product_categories as $product_category)
                                        <option value="{{ $product_category->id }}"
                                            {{ $product_category->id == $productSubCategory->product_category->id ? 'selected' : '' }}>
